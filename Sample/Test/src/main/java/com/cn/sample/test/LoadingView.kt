...
class LoadingView(context: Context, attrs: AttributeSet? = null) : View(context, attrs) {
    ...
    init {
        attrs?.let {
            context.obtainStyledAttributes(it, R.styleable.GradientRingView).apply {
                getColor(R.styleable.GradientRingView_gradientRingStartColor, Color.BLACK).let {
                    colors[0] = it
                }
                getColor(R.styleable.GradientRingView_gradientRingEndColor, Color.WHITE).let {
                    colors[1] = it
                }
                recycle()
            }
