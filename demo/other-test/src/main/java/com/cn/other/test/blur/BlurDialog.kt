...
/**
 * @author: cn
 * @time: 2026/4/10 16:08
 * @history
 * @description:
 */
class BlurDialog(context: Context, var blurType: Int) : AlertDialog(context) {

    //窗口背景高斯模糊程度，数值越高越模糊且越消耗性能
    private val mBackgroundBlurRadius = 90
    ...
    private fun setupWindowBlurListener() {
        val windowBlurEnabledListener: Consumer<Boolean> = Consumer { blursEnabled: Boolean -> this.updateWindowForBlurs(blursEnabled) }
    ...
    private fun updateWindowForBlurs(blursEnabled: Boolean) {
        Log.d(BlurDialog::class.simpleName, "updateWindowForBlurs: $blursEnabled, $blurType")
        if (blurType == BLUR_TYPE_BLUR_BACKGROUND) {
            //仅模糊背景
        ...
        if (blurType == BLUR_TYPE_BLUR_BEHIND) {
            //仅模糊后方屏幕
            window?.setDimAmount(if (blursEnabled) mDimAmountWithBlur else mDimAmountNoBlur) //调整背景周边昏暗的程度
