...
/**
 * @author: cn
 * @time: 24/7/2026 上午 11:26
 * @history
 * @description:
 */
class HomeFrag: BasicVmDBFragment<HomeFragViewModel, FragmentHomeBinding>(
    { it[HomeFragViewModel::class.java] },
    ...
    override fun onObserver() {
        lifecycleScope.launch {
            HomeModel.homeUiState.collectLatest { state ->
                binderMainAdapter.setDiffNewData(ArrayList(state.homeAppList))
            }
        }
    }
    ...
    class BinderHomePageApp: QuickDataBindingItemBinder<SoftEntity, ItemMainSoftBinding>() {
        ...
        @SuppressLint("NewApi")
        override fun convert(holder: BinderDataBindingHolder<ItemMainSoftBinding>, data: SoftEntity) {
            holder.run {
        ...
        override fun onClick(holder: BinderDataBindingHolder<ItemMainSoftBinding>, view: View, data: SoftEntity, position: Int) {
        }
    }
    ...
    class MainSoftAnimation: BaseAnimation {
        ...
        override fun animators(view: View): Array<Animator> {
            return arrayOf(
                ObjectAnimator.ofFloat(view, "scaleY", 1.5f, 1f).setDuration(DURATION).apply { DecelerateInterpolator() },
                ObjectAnimator.ofFloat(view, "scaleX", 1.5f, 1f).setDuration(DURATION).apply { DecelerateInterpolator() },
                ObjectAnimator.ofFloat(view, "alpha", 0.5f, 1f).setDuration(DURATION).apply { DecelerateInterpolator() },
                ObjectAnimator.ofFloat(view, "translationX", 250f, 0f).setDuration(DURATION).apply { DecelerateInterpolator() },
                ObjectAnimator.ofFloat(view, "translationY", 0f, 0f).setDuration(DURATION).apply { DecelerateInterpolator() }
            )
        }
    }
